...
try {
    ...
    $interactable = isset($_POST["interactable"]) ? $_POST["interactable"] : null; // Get target or null if not provided

    // Check if all required parameters are provided and valid
    if (is_null($sessionID) || is_null($x) || is_null($y) || is_null($z) || is_null($time) || is_null($interactable)) {
    ...
    $stmt = $conn->prepare("INSERT INTO `player_interactions`(`sessionID`, `x`, `y`, `z`, `time`, `interactable`) VALUES (?, ?, ?, ?, ?, ?)");
    if (!$stmt) {
        throw new Exception("Failed to prepare SQL statement: " . $conn->error);
    }

    // Bind parameters to the SQL query
    if (!$stmt->bind_param("idddds", $sessionID, $x, $y, $z, $time, $interactable)) {
